...
const createMockListStore = () => {
  ...
  const actions = {
    ...
    deleteList: async (listId: string) => {
      state.lists = state.lists.filter((l: MockList) => l.id !== listId);
    ...
    getList: (listId: string) =>
      state.lists.find((l: MockList) => l.id === listId),
    ...
    selectMultipleLists: (listIds: string[]) => {
      state.selectedListIds = [
        ...new Set([...state.selectedListIds, ...listIds]),
      ];
    },
    ...
    getSelectedLists: () => {
      return state.lists.filter((list: MockList) =>
        state.selectedListIds.includes(list.id)
      );
    },
    ...
    switchList: (listId: string) => {
      const list = state.lists.find((l: MockList) => l.id === listId);
      if (list) {
    ...
    getFavoriteLists: () => {
      return state.lists.filter((list: MockList) =>
        state.favorites.includes(list.id)
      );
    },
    ...
    getRecentLists: () => {
      return state.recent
        .map((id) => state.lists.find((l: MockList) => l.id === id))
        .filter(Boolean) as MockList[];
    },
    ...
    moveList: (listId: string, newPosition: number) => {
      const listIndex = state.lists.findIndex((l: MockList) => l.id === listId);
      if (listIndex >= 0) {
    ...
    getListStats: () => {
      return {
        ...
        byColor: state.lists.reduce((acc: any, list: MockList) => {
...
describe("ListStore Logic Tests", () => {
  ...
  describe("List CRUD Operations", () => {
    ...
    test("should update existing list", async () => {
      ...
      expect(updatedList.updatedAt.getTime()).toBeGreaterThan(
        createdList.updatedAt.getTime()
      );

      // Verify update in store
      const storedList = listStore.state.lists.find(
        (l) => l.id === createdList.id
      );
    ...
    test("should throw error when updating non-existent list", async () => {
      ...
      await expect(listStore.actions.updateList(updateData)).rejects.toThrow(
        "List not found"
      );
    });

    test("should delete existing list", async () => {
      const list1 = await listStore.actions.createList({
        name: "List 1",
        userId: "test-user-1",
      });
      const list2 = await listStore.actions.createList({
        name: "List 2",
        userId: "test-user-1",
      });
    ...
    test("should get list by ID", async () => {
      const list = await listStore.actions.createList({
        name: "Test List",
        userId: "test-user-1",
      });
  ...
  describe("List Selection and Current List", () => {
    ...
    beforeEach(async () => {
      list1 = await listStore.actions.createList({
        name: "List 1",
        userId: "test-user-1",
      });
      list2 = await listStore.actions.createList({
        name: "List 2",
        userId: "test-user-1",
      });
      list3 = await listStore.actions.createList({
        name: "List 3",
        userId: "test-user-1",
      });
    });
    ...
    test("should return selected lists", () => {
      ...
      expect(selectedLists.map((l) => l.id).sort()).toEqual([
        list1.id,
        list2.id,
      ]);
    });
  ...
  describe("Recent Lists Management", () => {
    ...
    test("limit recent lists to 10", async () => {
      const lists = [list1, list2, list3];

      // Create 12 unique lists to test the limit
      const uniqueLists: MockList[] = [];
      for (let i = 0; i < 12; i++) {
        const list = await listStore.actions.createList({
          name: `List ${i}`,
          userId: "test-user-1",
      ...
      // Add all to recent (this should trigger the limit)
      uniqueLists.forEach((list) => {
        listStore.actions.addToRecent(list.id);
      });

      expect(listStore.state.recent).toHaveLength(10); // Should be limited to 10
    });
  ...
  describe("List Search and Filtering", () => {
    beforeEach(async () => {
      await listStore.actions.createList({
        name: "Work List",
    ...
    test("should search lists by description", () => {
      ...
      expect(results.map((l) => l.name).sort()).toEqual([
        "Personal List",
        "Work List",
      ]);
    });
    ...
    test("should filter by favorites", async () => {
      ...
      const personalList = listStore.actions.getList(
        listStore.state.lists[1].id
      );
