 ...
 */
export const createTaskStore = (config?: { userId?: string }) => {
  ...
  return create<TaskStoreState & TaskStoreActions & TaskStoreSelectors>(
    persist(
      immer((set, get) => ({
        ...initialState,
        ...
        selectMultipleTasks: (taskIds: TaskId[]) => {
          set((state) => {
            state.selectedTaskIds = [
              ...new Set([...state.selectedTaskIds, ...taskIds]),
            ];
          });
        },
        ...
        removeListFilter: (listId: ListId) => {
          set((state) => {
            state.filters.listIds = state.filters.listIds.filter(
              (id) => id !== listId
            );
          });
        },
        ...
        batchUpdateTasks: async (
          ...
          for (const taskId of taskIds) {
            try {
              const updatedTask = await get().updateTask({
                id: taskId,
                ...updates,
              });
