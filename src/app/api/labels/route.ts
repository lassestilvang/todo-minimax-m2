...
import { withAuth, withRateLimit, withErrorHandling } from "../_lib/middleware";
import {
...
import { labelQuerySchema, createLabelSchema } from "../_lib/validation";
...
async function handleGetLabels(
  ...
  try {
    ...
    const labelsWithCounts = await dbAPI.getUserLabelsWithCounts(
      context.userId
    );
    ...
    const labelsWithStats = filteredLabels.map((label) => ({
      ...
      usagePercentage:
        label.taskCount > 0
          ? Math.round(
              (label.taskCount /
                labelsWithCounts.reduce((sum, l) => sum + l.taskCount, 0)) *
                100
            )
          : 0,
...
async function handleCreateLabel(
  ...
  try {
    ...
    const duplicateName = existingLabels.find(
    ...
    const duplicateCombo = existingLabels.find(
      (label) =>
    ...
    const completeLabel = completeLabels.find(
