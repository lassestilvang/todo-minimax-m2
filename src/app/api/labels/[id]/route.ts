...
import { updateLabelSchema, idParamSchema } from "../../_lib/validation";
...
async function handleGetLabel(
  ...
  try {
    ...
    const stats = {
      ...label,
      usagePercentage:
        totalTasks > 0 ? Math.round((label.taskCount / totalTasks) * 100) : 0,
      rankByUsage:
        labels
...
async function handleUpdateLabel(
  ...
  try {
    ...
    if (updateData.icon !== undefined || updateData.color !== undefined) {
      ...
      const duplicateCombo = labels.find(
        (label) =>
          label.id !== labelId &&
          label.icon === newIcon &&
        ...
        return NextResponse.json(
          {
            ...
            error: {
              code: "VALIDATION_ERROR",
              message:
                "A label with this icon and color combination already exists",
              statusCode: 400,
              timestamp: new Date().toISOString(),
            },
          },
    ...
    // Update label in database using direct query
    const updateFields = Object.entries(dbUpdateData).map(
      ([key, value]) => `${key} = ?`
    );

    const updateValues = [
      ...(Object.values(dbUpdateData) as any[]),
      new Date(),
      labelId,
    ];

    const sql = `UPDATE labels SET ${updateFields.join(
      ", "
    )}, updated_at = ? WHERE id = ?`;
    dbAPI.run(sql, updateValues);
...
async function handleDeleteLabel(
  ...
  try {
    ...
    if (existingLabel.taskCount > 0) {
      ...
      const tasksWithLabel = tasks.filter(
        (task) =>
    ...
    // Delete label from database using direct query
    dbAPI.run("DELETE FROM labels WHERE id = ?", [labelId]);
...
async function getLabelUsageStats(labelId: string, userId: string) {
  try {
    ...
    const recentlyUsed = tasksWithLabel.filter(
      (task) => new Date(task.updatedAt) > sevenDaysAgo
    ).length;
      ...
      firstUsed:
        tasksWithLabel.length > 0
          ? tasksWithLabel.reduce((oldest, task) =>
              oldest.createdAt < task.createdAt ? oldest : task
            ).createdAt
          : null,
      lastUsed:
        tasksWithLabel.length > 0
          ? tasksWithLabel.reduce((newest, task) =>
              newest.updatedAt > task.updatedAt ? newest : task
            ).updatedAt
          : null,
    };
  } catch (error) {
