...
const updateTaskSchema = z.object({
  ...
  estimate: z.string().regex(/^\d{2}:\d{2}$/).nullable().optional(),
  actualTime: z.string().regex(/^\d{2}:\d{2}$/).nullable().optional(),
  ...
  try {
    ...
    const updateData: UpdateTaskData = {
      ...
      dueDate: parsedData.dueDate !== undefined ? 
        (parsedData.dueDate ? new Date(parsedData.dueDate) : undefined) : 
        undefined,
    };
