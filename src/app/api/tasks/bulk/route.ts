  ...
  try {
    ...
    switch (operation) {
      ...
      case "delete":
        ...
        for (const taskId of taskIds) {
          try {
            const deleted = await dbAPI.deleteTask(taskId, "default-user");
            deleteResults.push({
              taskId,
              success: deleted !== null, // Check if operation didn't throw error
            });
          } catch (error) {
            deleteResults.push({
              taskId,
      ...
      case "move":
        ...
        for (const taskId of taskIds) {
          try {
            ...
            const movedTask = await dbAPI.updateTask(
              taskId,
              moveUpdateData,
              "default-user"
            );
        ...
        for (const taskId of taskIds) {
          try {
            ...
            const duplicateData = {
              ...
              status: "todo" as const, // Cast to TaskStatus
