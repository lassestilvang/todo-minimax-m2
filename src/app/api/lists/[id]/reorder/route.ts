...
async function handleReorderList(req: NextRequest, context: ApiContext, listId: string): Promise<NextResponse> {
  try {
    ...
    const missingTasks = taskIds.filter(id => !currentTaskIds.includes(id));
    ...
    if (reorderAll) {
      ...
      for (const taskId of taskIds) {
        try {
          await dbAPI.updateTask(taskId, { position: currentPosition }, context.userId);
          const updatedTask = await dbAPI.getTaskWithDetails(taskId);
          if (updatedTask) {
      ...
      for (const task of allTasksWithPositions) {
        try {
          await dbAPI.updateTask(task.id, { position: currentPosition }, context.userId);
          ...
          console.error(`[List Reorder] Error updating task ${task.id}:`, error);
        }
      }
