...
export async function GET(request: NextRequest) {
    ...
    if (parsedFilters.isFavorite !== undefined) {
      filteredLists = lists.filter(
        (list) => list.isFavorite === parsedFilters.isFavorite
            );
    }
    ...
    const listsWithCounts = await Promise.all(
      paginatedLists.map(async (list) => {
        const listTasks = await dbAPI.getUserTasks("default-user", {
          ...
          completedTasks: listTasks.filter((t) => t.status === "done").length,
          pendingTasks: listTasks.filter((t) => t.status === "todo").length,
          inProgressTasks: listTasks.filter((t) => t.status === "in_progress")
            .length,
        };
      })
    );
...
export async function POST(request: NextRequest) {
    ...
    const nextPosition =
      existingLists.length > 0
            ? Math.max(...existingLists.map((l) => (l.position || 0))) + 1
        : 0;
